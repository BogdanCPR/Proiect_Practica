...
def update_data(num):
    ...
    output = subprocess.check_output(['/home/bogdan/network.sh']).decode('utf-8').strip().split('\t')[-1].split()
...
# crearea animației
ani = animation.FuncAnimation(fig, update_data, interval=1000, blit=True)
